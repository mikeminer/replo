export class ReploClient {
  ...
  me = () => this.request<{ organization: { id: string; name: string; plan: "free" | "startup" | "partner" | "enterprise" } }>("/v1/me");
  research = (url: string) => this.request<{ jobId: string }>("/v1/research/from-url", { method: "POST", body: JSON.stringify({ url }) });
  discoverProspects = (input: { url: string; audience?: string; territory?: string; limit?: number }) => { const query = new URLSearchParams(Object.entries(input).filter((entry): entry is [string, string | number] => entry[1] !== undefined).map(([key, value]) => [key, String(value)])); return this.request<DiscoveryResult>(`/v1/prospects/discover?${query}`, { signal: AbortSignal.timeout(95_000) }); };
...
export type DiscoveryResult = { analysis: { url: string; name: string; title: string; summary: string; keywords: string[] }; query: string; prospects: DiscoveryProspect[]; sourcesScanned: number; partial?: boolean; strategy: { buyerProfile: { target: string; decisionMakers: string; source: "provided" | "inferred" }; competitorPolicy: "exclude_competing_vendors" | "buyer_override" | "not_applicable"; channels: DiscoveryChannel[]; contextSources: DiscoveryContextSource[]; contactPolicy: "official_company_sites_only" } };
