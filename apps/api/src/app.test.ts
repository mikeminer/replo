...
describe("Replo API", () => {
  ...
  it("launches selected prospects atomically with the configured workspace key", async () => { process.env.REPLO_WORKSPACE_API_KEY = "rk_test_persistent"; const sender = new MockSender(), { app } = createApp({ sender }); const response = await app.request("/v1/campaigns/launch-from-prospects", { method: "POST", headers: { authorization: "Bearer rk_test_persistent", "content-type": "application/json" }, body: JSON.stringify({ name: "Autonomous GTM", sequence: [{ subject: "Hi", body: "Hello", delayDays: 0 }], prospects: [{ firstName: "Ada", lastName: "Lovelace", domain: "example.com", knownEmail: "ada@example.com", companyName: "Example" }], acceptedReplyLock: true }) }); expect(response.status).toBe(200); expect(await response.json()).toMatchObject({ status: "active", provider: "mock" }); });
});
