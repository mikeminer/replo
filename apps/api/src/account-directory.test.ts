...
describe("SupabaseAccountDirectory", () => {
  it("hashes API keys before authorization and returns the organization", async () => {
    const rpc = vi.fn().mockResolvedValue({ data: [{ organization_id: "org_1", organization_name: "Acme", organization_plan: "pro", monthly_send_count: 0, monthly_resolve_count: 0, remaining: 9999 }], error: null });
    ...
    expect(result).toMatchObject({ org: { id: "org_1", name: "Acme", plan: "pro" }, remaining: 9999 });
  });
